        ...
        for k in range(0, self.k_train):
            ...
            for t in range(self.train_tasks):
                ...
                for batch_idx, batch in enumerate(tqdm(support_sets[t], desc="Iteration")):
                    ...
                    support_loss = torch.sum(self.criterion(graph_pred.double(), label.to(torch.float64)))/graph_pred.size(dim=0)
                    loss_support += support_loss
                                   
                    if self.baseline == 0:
                        pred = self.transformer(self.gnn.pool(emb, batch.batch))
                        inner_loss = torch.sum(self.criterion_transformer(F.sigmoid(pred).double(), label.to(torch.float64)))/pred.size(dim=0)
                ...
                for batch_idx, batch in enumerate(tqdm(query_sets[t], desc="Iteration")):
                    ...
                    graph_pred, emb = self.gnn(batch.x, batch.edge_index, batch.edge_attr, batch.batch)                    
                    label = batch.y.view(graph_pred.shape)
                    query_loss = torch.sum(self.criterion(graph_pred.double(), label.to(torch.float64)))/graph_pred.size(dim=0)
                    loss_query += query_loss
                    
                    if self.baseline == 0:
                        logit = self.transformer(self.gnn.pool(emb, batch.batch))
                        outer_loss = torch.sum(self.criterion_transformer(F.sigmoid(logit).double(), label.to(torch.float64)))/logit.size(dim=0)
